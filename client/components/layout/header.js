	...
	const LoginPopup = (
		<Modal isOpen={isOpen} onClose={onClose} size={'lg'}>
			...
			<ModalContent>
				<ModalCloseButton />
				<ModalBody
					className={'unw-login-form'}
					minHeight={!forgetPassword ? '500px' : '420px'}>
					<Text
						fontSize={24}
						marginTop={'20px'}
						textAlign={'center'}
					...
					{!forgetPassword ? (
							...
							<div className='h-auto max-w-[400px] bg-white'>
								<div className='text-center'>
									...
									<div className='p-4 text-lg'>
										Mới sử dụng lần đầu?{' '}
										<a className='text-[#F98820] cursor-pointer'>
											Đăng Ký Ngay
										</a>
									</div>
									<Box height={5}></Box>
						...
						<Stack>
							<Text className={'title'} marginTop={'30px'}>
								Nhập email/sdt của bạn để khôi phục mật khẩu của bạn. bạn sẽ
								nhận được một tin nhắn có hướng dẫn để giúp bạn giải quyết các
								vấn đề của mình
							</Text>
							<Input
								className={'unw-input'}
							...
							<Text
								className='unw-forget-password-return'
								onClick={changeForgetPassword}>
				...
				<Flex alignItems={'center'}>
					<Box className='item'>
						<Text
							fontSize={16}
							fontWeight={'bold'}
							color={router.pathname === '/' ? '#FF6B00' : '#000'}>
					...
					<Box className='item'>
						<Text
							...
							fontWeight={'bold'}
							color={
								router.pathname.includes('/information') ? '#FF6B00' : '#000'
							}>
					...
					<Box className='item'>
						<Text
							fontSize={16}
							fontWeight={'bold'}
							color={router.pathname.includes('/job') ? '#FF6B00' : '#000'}>
							<Link href={'#'}>Tìm việc làm</Link>
						</Text>
					</Box>
					<Box className='item'>
						<Text>
							<Image src={Avatar} onClick={onOpen} />
						</Text>
					</Box>
				</Flex>
			</Flex>
